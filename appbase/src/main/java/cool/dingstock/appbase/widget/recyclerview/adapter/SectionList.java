...
class SectionList<I extends BaseItem> implements Cloneable {
    ...
    public int getViewType(@IntRange(from = 0) int position) {
        Section section = findSection(position);
        if (null == section) {
            Logger.e("getViewType  position="+position );
            return -1;
        }
    ...
    public List<I> getAllItemViewList(){
        if (null==this.mSectionList){
            return  null;
        }
        List<I> itemList=new ArrayList<>();
        for (int index = 0; index < this.mSectionList.size(); index++) {
            Section section = this.mSectionList.get(index);
            if (null==section){
                continue;
            }
            itemList.addAll(section.getItemViewList());
        }
        return itemList;
    }
