package cool.dingstock.appbase.widget.consecutivescroller;
...
public class ConsecutiveScrollerLayout extends ViewGroup implements ScrollingView, NestedScrollingParent2, NestedScrollingChild2 {
	...
	int mScrollRange;
	/**
	 * 记录布局垂直的偏移量，它是包括了自己的偏移量(mScrollY)和所有子View的偏移量的总和， 这个值不是真实的布局滑动偏移量，只是用于在滑动是记录和计算每次的滑动距离。
	 */
	...
	/**
	 * 是否触摸吸顶view并且不能触发布局滑动 注意：它不仅会判断自己的吸顶view，也会判断下级ConsecutiveScrollerLayout的吸顶view
	 */
...
//        // 导致ConsecutiveScrollerLayout追踪布局的滑动出现偏差
	...
	/**
	 * 禁止设置滑动监听，因为这个监听器已无效 若想监听容器的滑动，请使用
	 *
	 * @param l
	 *
	...
	public static class LayoutParams extends ViewGroup.MarginLayoutParams {
		...
		/**
		 * 是否与父布局整体滑动，设置为false时，父布局不会拦截它的事件，滑动事件将由子view处理。 可以实现子view内部的垂直滑动。
		 */
		public boolean isConsecutive = true;
		
		/**
		 * 是否支持嵌套滑动，默认支持，如果子view或它内部的下级view实现了NestedScrollingChild接口， 它可以与ConsecutiveScrollerLayout嵌套滑动，把isNestedScroll设置为false可以禁止它与ConsecutiveScrollerLayout嵌套滑动。
		 */
		...
		/**
		 * 在View吸顶的状态下，是否可以触摸view来滑动ConsecutiveScrollerLayout布局。 默认为false，则View吸顶的状态下，不能触摸它来滑动布局
		 */
		public boolean isTriggerScroll = false;
		
		/**
		 * 吸顶下沉模式 默认情况下，吸顶view在吸顶状态下，会显示在布局上层，覆盖其他布局。 如果设置了下沉模式，则会相反，view在吸顶时会显示在下层，被其他布局覆盖，隐藏在下面。
		 */
