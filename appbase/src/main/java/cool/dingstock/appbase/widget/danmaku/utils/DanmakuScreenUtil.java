package cool.dingstock.appbase.widget.danmaku.utils;
 ...
 * <p>
 * 自动适配屏幕像素的工具类。
 * 需要先调用{@link DanmakuScreenUtil#init(Context)}才能正常使用。如果屏幕旋转，
 * 那么需要再次调用{@link DanmakuScreenUtil#init(Context)}以更新。
 */

@SuppressWarnings({"unused", "WeakerAccess", "SuspiciousNameCombination"})
public class DanmakuScreenUtil {
    private static final String TAG = "ScreenUtil";
    /**
     * 屏幕宽度，在调用init()之后通过{@link DanmakuScreenUtil#getScreenWidth()}获取
     */
    private static int sScreenWidth = 1920;

    /**
     * 屏幕高度，在调用init()之后通过{@link DanmakuScreenUtil#getScreenHeight()} ()}获取
     */
    private static int sScreenHeight = 1080;

    /**
     * 设计宽度。用于{@link DanmakuScreenUtil#autoWidth(int)}
     */
    private static int sDesignWidth = 1080;

    /**
     * 设计高度。用于{@link DanmakuScreenUtil#autoHeight(int)} (int)}
     */
