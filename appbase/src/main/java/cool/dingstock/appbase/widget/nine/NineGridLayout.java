...
public abstract class NineGridLayout extends ViewGroup {
    ...
    private static final float DEFUALT_SPACING = 8f;
    ...
    private List<CircleImageBean> mUrlList = new ArrayList<>();
    ...
    public NineGridLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            TypedArray typedArray = getContext().obtainStyledAttributes(attrs, R.styleable.NineGridLayout);
            mSpacing = typedArray.getDimension(R.styleable.NineGridLayout_layout_space, DEFUALT_SPACING);
            typedArray.recycle();
        }
    ...
    public void setUrlList(List<CircleImageBean> urlList) {
    ...
    private void refresh() {
        ...
        if (size == 1) {
            CircleImageBean url = mUrlList.get(0);
            RoundImageView imageView = createImageView(0, url);
        ...
        for (int i = 0; i < size; i++) {
            CircleImageBean url = mUrlList.get(i);
    ...
    private RoundImageView createImageView(final int i, final CircleImageBean url) {
        RoundImageView imageView = new RoundImageView(mContext);
        imageView.setType(RoundImageView.TYPE_ROUND);
        imageView.setBorderWidth(SizeUtils.dp2px(1f));
        imageView.setBorderCorner(SizeUtils.dp2px(4));
        imageView.setBorderColor(Color.parseColor("#F7F7F9"));
     ...
     */
    private void layoutImageView(RoundImageView imageView, int i, CircleImageBean url, boolean showNumFlag) {
    ...
    private int getListSize(List<CircleImageBean> list) {
     ...
     */
    protected abstract boolean displayOneImage(RoundImageView imageView, CircleImageBean url, int parentWidth);

    protected abstract void displayImage(RoundImageView imageView, CircleImageBean url);

    protected abstract void onClickImage(int position, CircleImageBean url, List<CircleImageBean> urlList);
}
