 ...
 * @Description:
 */
class SkipDecoration(var divider: Drawable, var skipPosition: Int = 0) : androidx.recyclerview.widget.RecyclerView.ItemDecoration() {
    ...
    private fun drawVertical(canvas: Canvas, parent: androidx.recyclerview.widget.RecyclerView) {
        ...
        for (i in 0 until childCount - skipPosition) {
    ...
    override fun getItemOffsets(outRect: Rect, view: View, parent: androidx.recyclerview.widget.RecyclerView,
        ...
        if (getOrientation(parent.layoutManager!!) == androidx.recyclerview.widget.LinearLayoutManager.VERTICAL) {
            outRect.set(0, divider.intrinsicHeight * startScale, 0, divider.intrinsicHeight * endScale)
        } else {
            outRect.set(divider.intrinsicWidth * startScale, 0, divider.intrinsicWidth * endScale, 0)
        }
    }
