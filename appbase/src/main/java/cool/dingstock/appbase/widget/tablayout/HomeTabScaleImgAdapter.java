...
public class HomeTabScaleImgAdapter extends CommonNavigatorAdapter {
    ...
    public HomeTabScaleImgAdapter(List<TabTitleEntity> titleList) {
    ...
    public IPagerTitleView getTitleView(Context context, int i) {
        ...
        HomeTabTitleView titleView = new HomeTabTitleView(context);
        titleView.getTabTitleView().setNormalColor(normalColor != 0 ? normalColor : ContextCompat.getColor(context, R.color.color_text_black4));
        titleView.getTabTitleView().setSelectedColor(selectedColor != 0 ? selectedColor : ContextCompat.getColor(context, R.color.color_text_black1));
        titleView.getTabTitleView().setText(titleList.get(i).getTitle());
        titleView.setMaxScale(endTitleSize / startTitleSize);
        titleView.setOnClickListener(view -> {
            if (null != mTabSelectListener) {
                mTabSelectListener.onSelected(i);
            }
        });
        titleView.getTabTitleView().setTextSize(startTitleSize);
        titleView.getTabTitleView().setStartAndEndTitleSize(startTitleSize, endTitleSize);
        titleView.setPadding(padding);
        titleView.setupIcon(tabTitleEntity.getTip());
        switch (titleModel) {
            case Bottom:
                titleView.getTabTitleView().setGravity(Gravity.BOTTOM);
                break;
            case Center:
                titleView.getTabTitleView().setGravity(Gravity.CENTER);
                break;
        }
        return titleView;
    }
