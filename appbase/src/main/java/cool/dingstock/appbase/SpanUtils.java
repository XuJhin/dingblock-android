...
public class SpanUtils {
    ...
    private NoRefCopySpan clickSpan;
    private String url;
     ...
     * @return the single {@link SpanUtils} instance
     */
    public SpanUtils setClickSpan(@NonNull final NoRefCopySpan clickSpan) {
    ...
    private void updateCharCharSequence() {
        ...
        int start = mBuilder.toString().length();
        ...
        int end = mBuilder.toString().length();
        if (start == end) {
            return;
        }
        if (verticalAlign != -1) {
    ...
    static class CustomImageSpan extends CustomDynamicDrawableSpan {
        ...
        private CustomImageSpan(final Bitmap b, final int verticalAlignment) {
            ...
            mDrawable = new BitmapDrawable(AppUtils.INSTANCE.getApplicationContext().getResources(), b);
            mDrawable.setBounds(
        ...
        public Drawable getDrawable() {
            ...
            if (mDrawable != null) {
                ...
                try {
                    InputStream is =
                            AppUtils.INSTANCE.getApplicationContext().getContentResolver().openInputStream(mContentUri);
                    bitmap = BitmapFactory.decodeStream(is);
                    drawable = new BitmapDrawable(AppUtils.INSTANCE.getApplicationContext().getResources(), bitmap);
                    drawable.setBounds(
                ...
                try {
                    drawable = ContextCompat.getDrawable(AppUtils.INSTANCE.getApplicationContext(), mResourceId);
                    drawable.setBounds(
                            0, 0, drawable.getIntrinsicWidth(), drawable.getIntrinsicHeight()
                    );
                } catch (Exception e) {
