...
/**
 * A class used to manipulate the {@link }'s indicator {@link Drawable} at any
...
class TabIndicatorInterpolator {
     ...
     * (if present).
     *
     * @param tabView  {@link DcTabLayout.TabView} for which to calculate left and right content bounds.
    ...
    static RectF calculateTabViewContentBounds(
            @NonNull DcTabLayout.TabView tabView, @Dimension(unit = Dimension.DP) int minWidth) {
        int tabViewContentWidth = tabView.getContentWidth();
        int tabViewContentHeight = tabView.getContentHeight();
        int minWidthPx = (int) SizeUtils.dp2px( minWidth);
     ...
     */
    static RectF calculateIndicatorWidthForTab(DcTabLayout tabLayout, @Nullable View tab) {
        if (tab == null) {
            return new RectF();
        }

        // If the indicator should fit to the tab's content, calculate the content's widtd
        if (!tabLayout.isTabIndicatorFullWidth() && tab instanceof DcTabLayout.TabView) {
            return calculateTabViewContentBounds((DcTabLayout.TabView) tab, MIN_INDICATOR_WIDTH);
        }
     ...
     */
    void setIndicatorBoundsForTab(DcTabLayout tabLayout, View tab, @NonNull Drawable indicator) {
