...
import cool.dingstock.imagepre.tool.common.HandlerUtils;
...
public class ImagePreviewActivity extends AppCompatActivity implements Handler.Callback, View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        bgColor = ImagePreview.getInstance().getBgColor();
        ...
        isShowOriginButton = ImagePreview.getInstance().isShowOriginButton(this,currentItem);
        ...
        viewPager.addOnPageChangeListener(new ViewPager.SimpleOnPageChangeListener() {
            ...
            public void onPageSelected(int position) {
                ...
                isShowOriginButton = ImagePreview.getInstance().isShowOriginButton(ImagePreviewActivity.this,currentItem);
                if (isShowOriginButton) {
                    // 检查缓存是否存在
                    checkCache(currentItemOriginPathUrl);
                } else {
    ...
    private void downloadCurrentImg() {
        DownloadPictureUtil.downloadPicture(context.getApplicationContext(), currentItemOriginPathUrl,imagePreviewAdapter.getGifView(currentItemOriginPathUrl)
    ...
    public int convertPercentToBlackAlphaColor(float percent) {
        ...
        String color = "#" + (stringAlpha.length() < 2 ? "0" : "") + stringAlpha + bgColor;
    ...
    public boolean handleMessage(Message msg) {
        if (msg.what == 0) {// 点击查看原图按钮，开始加载原图
            ...
            if(path.endsWith(".gif")){
                PhotoView gifView = imagePreviewAdapter.getGifView(path);
    ...
    private void downGif(String path, PhotoView gifView) {
        Glide.with(this).downloadOnly().load(path).into(new FileTarget(){
            @Override
            public void onResourceReady(@NonNull File resource, @Nullable Transition<? super File> transition) {
                super.onResourceReady(resource, transition);
                Glide.with(ImagePreviewActivity.this).load(resource).into(gifView);
                gifView.setVisibility(View.VISIBLE);
                SubsamplingScaleImageViewDragClose imgView = imagePreviewAdapter.getImgView(path);
