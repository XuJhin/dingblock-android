...
import cool.dingstock.imagepicker.activity.PickerActivityManager;
...
public class MultiImagePreviewActivity extends AppCompatActivity implements MediaItemsDataSource.MediaItemProvider {
    ...
    public static void intent(Activity activity, ImageSet imageSet,
        ...
        intent.putExtra(MultiImagePickerActivity.INTENT_KEY_CURRENT_INDEX, position);
    ...
    private boolean isIntentDataFailed() {
        if (getIntent() == null || !getIntent().hasExtra(MultiImagePickerActivity.INTENT_KEY_SELECT_CONFIG)
                || !getIntent().hasExtra(MultiImagePickerActivity.INTENT_KEY_PRESENTER)) {
            return true;
        }
        selectConfig = (MultiSelectConfig) getIntent().getSerializableExtra(MultiImagePickerActivity.INTENT_KEY_SELECT_CONFIG);
        presenter = (IPickerPresenter) getIntent().getSerializableExtra(MultiImagePickerActivity.INTENT_KEY_PRESENTER);
        mCurrentItemPosition = getIntent().getIntExtra(MultiImagePickerActivity.INTENT_KEY_CURRENT_INDEX, 0);
