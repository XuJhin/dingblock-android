...
import cool.dingstock.imagepicker.utils.PStatusBarUtil;
...
public abstract class PBaseLoaderFragment extends Fragment implements ICameraExecutor {
    ...
    public void takePhoto() {
        ...
        if (ContextCompat.checkSelfPermission(getActivity(), Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
            requestPermissions(new String[]{Manifest.permission.CAMERA}, ImagePicker.REQ_CAMERA);
        } else {
    ...
    protected void loadMediaSets() {
        ...
        if (ContextCompat.checkSelfPermission(getActivity(), Manifest.permission.WRITE_EXTERNAL_STORAGE)
            ...
            requestPermissions(new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE}, ImagePicker.REQ_STORAGE);
        } else {
            //从媒体库拿到文件夹列表
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions,
        ...
        if (requestCode == ImagePicker.REQ_CAMERA) {
        ...
        } else if (requestCode == ImagePicker.REQ_STORAGE) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                //申请成功，可以拍照
                loadMediaSets();
            } else {
