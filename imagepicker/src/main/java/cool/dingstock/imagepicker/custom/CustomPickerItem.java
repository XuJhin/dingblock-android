...
public class CustomPickerItem extends PickerItemView {
    ...
    @Override
    public void enableItem(ImageItem imageItem, boolean isChecked, int indexOfSelectedList, boolean hasArrivedMax) {
        ...
        if (hasArrivedMax) {
            mVMasker.setVisibility(VISIBLE);
        } else {
            mVMasker.setVisibility(isChecked ? VISIBLE : GONE);
        }
        mVMasker.setBackgroundColor(isChecked ? Color.parseColor("#80000000") : Color.parseColor("#80ffffff"));
    }
